...
public class ToyFactory {
	...
	public void displayInfo() {
		...
		do {
			...
			try {										// InputMismatch 에러 잡는 용도
				...
				case 0: 
					System.out.println("공장 관리를 종료합니다");
					menuNum = 0;
					break;
				}
	...
	public void createToy() throws InputMismatchException {
		...
		while(true) {									// q 가 입력될 때 까지 무한 반복
			...
			addNum = sc.next();							// 숫자, 문자 다 있으므로 String 타입으로 받고 parseInt 사용
			...
			for(Entry<Integer, String> entry : materialMap.entrySet()) {		// 제대로 재료 번호 입력한 경우
				if(entry.getKey() == Integer.parseInt(addNum)) {
					addToyMaterial.add(materialMap.get(Integer.parseInt(addNum)));
					System.out.println("재료가 추가되었습니다 : " + entry.getValue());
					break;								// 찾아서 추가했으니 for 문 탈출
				}
			}											// for 문 끝
		}												// while 문 끝
	...
	public void sortByDay() {
		...
		List<Toy> toysCopied = new ArrayList<Toy>();	// 원본 유지를 위한 복사본 생성
		
		for(Toy toy : toys) {							// 원본에 있는 장난감 그대로 추가
			toysCopied.add(toy);
		}
